...
function AddressForm({ onNext, onPrev }) {
  ...
  const handleNext = async () => {
    // If a selected address is available, proceed with that address
    if (selectedAddress) {
      const addressData = {
      ...
      // If no selected address, you can handle it in some way or show a message
  ...
  const handleDeliveryHere = async (orderId) => {
    try {
      // Find the selected address based on the orderId
      const selectedAddress = addresses.find((address) => address.orderId === orderId);
  
      // Update the state with the selected address and order ID
      setSelectedAddress(selectedAddress);
      setSelectedOrderId(orderId);
  
      // Proceed to the next step (you can call handleNext here)
      handleNext();
  ...
  const handleFirstNameChange = (e) => {
    ...
    // Basic validation: non-empty first name
    if (!value) {
