...
const LoginSignup = () => { 
  ...
  const handleSubmit = async (e) => {
    ...
    const isCustomerNameExists = existingCustomers.some(customer => customer.customerName === customerName);
    const isContactNoExists = existingCustomers.some(customer => customer.contactNo === contactNo);
    const isEmailExists = existingCustomers.some(customer => customer.email === email);

   
    if (isCustomerNameExists) {
      toast.error("Error: Customer name already exists", {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:8080/api/customer/save", formDataObject, {
        headers: {
