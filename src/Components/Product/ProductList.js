...
const addToCart = async (productId) => {
  ...
  if (userData?.customerId) {
    try {
      const req = { productId, quantity: 1 };
    ...
    try {
      ...
      if (!productData.productName) {
        toast.error("Not added to cart, try again", {
          position: "top-right",
          autoClose: 3000,
        ...
        const { productName, productCost, discountPercent, discountedPrice, pImage } = productData;
        ...
        const updatedCartItems = [
          ...cartItems,
          {
            ...
            price: productCost,
            discountedPrice,
            product: { productName, discountPercent, pImage },
          },
        ];

        setCartItems(updatedCartItems);
        localStorage.setItem("cartItems", JSON.stringify(updatedCartItems));
        console.log(`Product ${productId} added to cart locally for logged-out user`, JSON.stringify(updatedCartItems));
      }
    } catch (error) {
      console.error("Error", error);
    }
  ...
  <div className="card filter-options" style={{ backgroundColor: 'black' }}>
    <h4 style={{ color: 'white' }}>Filter Options</h4>
        ...
        {categories.map((category) => (
          <optgroup key={category.categoryId} label={category.categoryNames}>
            {category.subcategories.map((element) => (
              <option key={element.subId} value={element.subId}>
    ...
    <h4 style={{ color: 'white', marginTop: '20px' }}>Pricing Filter:</h4>
    <div className="form-group" style={{ color: 'white' }}>
      ...
      <div>
        ...
        <span style={{ marginLeft: '5px' }}> &nbsp; Max Price: ₹{maxPrice}</span>
      </div>
    </div>
  </div>
</div>
