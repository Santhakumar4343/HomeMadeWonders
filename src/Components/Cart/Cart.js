...
function Cart() {
  ...
  const cartData = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/api/cart/carts/${userData.customerId}`);
  ...
  const deleteItemFromCart = async (cartItemId, productId) => {
  try {
    // Send a delete request to the backend
    await axios.delete(`http://localhost:8080/api/cartitem/${cartItemId}/${userData.customerId}`);

    // After successful delete from the backend, update the cart data in your component
    cartData();
        ...
        console.error('Error:', error);
              ...
              // Create a new array without the item with the specified productId
              const updatedCartItems = storedCartItems.filter((item) => item.productId !== productId);
      ...
      <Row>
        <Col xs={12} md={8}>
          <div className="cart-items">
            <h3>Cart Items:</h3>
            {customerCartDetails.cartItems && customerCartDetails.cartItems.length > 0 ? (
                ...
                <Card.Body>
                  ...
                  { (cartItem.quantity > 1) ? 
                    ...
                    (  <button
                      ...
                      onClick={() => deleteItemFromCart(cartItem.id,cartItem.productId)}
                      style={{ color : 'red'}}
                    >
                     &#128465; 
                    </button>)
                    ...
                    <div className="col-7 text-center mt-4 ml-3 ">
        ...
        <Col xs={12} md={4}>
          ...
          <div className="card mt-1">
            ...
            <p style={{color: 'green' , paddingTop: '30px'}}>You will save  ₹{customerCartDetails.discounte} on this order</p>
          </div>
